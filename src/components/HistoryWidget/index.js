...
import { getMyBookingHistory } from "../../services/me.service";
import Pagination from "../Pagination";
import { SUCCESS, FAILED, CANCELED } from "../../constants/bookingStatus";
import { getMyMatch } from "../../services/me.service";
import { EMPTY } from "../../constants/default";
import { decrypt, encryptKey } from "../../helpers/crypto.helper";
import { OWNER, USER } from "../../constants/roles";
import { getBookingHistoryByOwner } from "../../services/owner.service";
...
function HistoryWidget() {
  ...
  const getHistoryBooking = (page = 1, itemsPerPage = ITEMS_PER_PAGE) => {
    ...
    if (credential) {
      ...
      if (role === OWNER) {
        getBookingHistoryByOwner({ page, itemsPerPage })
          .then((res) => {
        ...
        getMyBookingHistory({ page, itemsPerPage })
          .then((res) => {
            setHistoryBookingModels(res.data);
            setMaxPage(
              res.maxResult % ITEMS_PER_PAGE === 0 && res.maxResult !== 0
        ...
        isLoading ? (
          ...
          <>
            <table className="table">
              <thead>
                <tr>
                  <th scope="col" style={{ width: "15%" }}>Yard</th>
                  <th scope="col" style={{ width: "10%" }}>Sub-yard</th>
                  ...
                  <th scope="col" style={{ width: "8%" }}>Created By</th>
                ...
                <>
                  {
                    historyBookingModels.map(historyBooking => (
                      <tr>
                        ...
                        <td className="text-truncate" title={historyBooking.address}>{historyBooking.address}</td>
                        <td>{historyBooking.price} VND</td>
                        <td className={getColor(historyBooking.bookingStatus)}
                        >{historyBooking.bookingStatus}</td>
                        <td className="text-truncate" title={historyBooking.note}>{historyBooking.note || "N/A"}</td>
