...
function Yard() {
  const { id } = useParams();
  const item = yards.find((yard) => yard.id === id);
  const [slideImages, setSlideImages] = useState(item.images);
      ...
      <div className="row flex-row">
        ...
        <div className="col-4 ps-4 pe-4 pt-5 flex-column">
          <div className="text-center mb-3">
            <b className="size-2 d-block">{item.name}</b>
          ...
          <div className="row mb-1 yard__details-field">
            ...
            <span className="col-9">{item.address}</span>
          </div>
          <div className="row mb-1 yard__details-field">
            <span className="col-3 fw-bolder">Open:</span>
            <span className="col-9">
              {TIMELINE.find((time) => time.value === item.openAt).label} -{" "}
              {TIMELINE.find((time) => time.value === item.closeAt).label}
            </span>
          </div>
        </div>
      </div>
      <div className="row justify-content-center mt-5">
        <div className="col-7 pt-4">
          ...
          <div className="row ps-2">
            {item.subYards[0].slots.map((slot) => (
        ...
        <div className="col-5 ps-4 pe-4 flex-column pt-4">
          ...
          <div className="pt-3 pb-3 justify-content-around">
            <button className="btn btn-primary p-2 w-100" onClick={onBooking}>
              Booking
            </button>
          </div>
        </div>
